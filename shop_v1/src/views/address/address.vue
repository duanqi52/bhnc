<template>
  <div class="orderInfo">
    ...
    <div class="address  w">
      ...
      <ul>
        <li v-for="(item,index) in addressList" :key="index" @click="changeList(index)">
          <div class="chooseBox">
            <input
              type="checkbox"
              class="chooseBtn"
          ...
          <div class="chooseType1" v-else @click="setDefault(item)">设置默认</div>
        </li>
      </ul>
      <div></div>
      <!-- 添加地址按钮 -->
      ...
      <!-- 添加地址先隐藏，点击时显示 -->
    <div class="addressBox w" v-show="isShow" style="marginTop: 42px;">
      <ul>
        <li >
          <i>收件人：</i>
        ...
        <li>
          ...
          <input type="button" value="取消" class="cdBtn" @click="cancelAdd" />
        </li>
      </ul>
    </div>
