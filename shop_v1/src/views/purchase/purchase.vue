  ...
  <div id="bhncPurchase">
    <div class="hadeer_tou">
      <div class="hadeer_tou_can cans" style="color: #D22D36;background-color: white;">
        <!-- 分类 -->
        <!-- <span class="classify" @click="show3 = !show3">
          <span class="classify_a">分类▼</span>
        </span>
        <div class="tangchu">
          <el-collapse-transition>
            <div
              v-show="show3"
              style="padding: 13px; border: 1px solid #4ca5e9;line-height: 40px;min-width: 555px;"
              c
            >
              <div class="classify_s">
                <el-collapse v-model="activeNames" >
                  <el-collapse-item title="成品笔" name="1">
                    <el-tree :data="data" show-checkbox node-key="id" :props="defaultProps"></el-tree>
                  </el-collapse-item>
                  <el-collapse-item title="笔配件" name="2">
                    <el-tree :data="data" show-checkbox node-key="id" :props="defaultProps"></el-tree>
                  </el-collapse-item>
                  <el-collapse-item title="笔芯" name="3">
                    <el-tree :data="data" show-checkbox node-key="id" :props="defaultProps"></el-tree>
                  </el-collapse-item>
                </el-collapse>
                <el-checkbox
              ...
              <div class="but_que">
                <el-button @click="show3 = !show3" type="primary" plain>重置</el-button>
                <el-button @click="show3 = !show3;typeSearch()" type="primary">确定</el-button>
              </div>
            </div>
          </el-collapse-transition>
        </div>-->
    ...
    <div class="hadeer_tou_can">
      <ul class="purchaseOrder">
        <li
          ...
          :key="index"
          @click="toPurchaseDetail(purchase)"
        >
          <div :id="'img'+index" v-html="purchase.content_img" v-show="false"></div>
          <p class="purchaseOrder_left_a">{{purchase.title}}</p>
          <div class="purchaseOrder_left">
            <p class="purchaseOrder_left_b">{{purchase.content_img.content}}</p>
            <!-- <ul class="purchaseOrder_left_img">
              <li v-for="(img,index) in purchase.imgs" :key="index">
                <img :src="img" alt />
              </li>
            </ul>-->
            <div class="purchaseOrder_right">
              <div style="text-align: center;">
                <img
                  src="../../../static/img/immediately.png"
                  alt
                  class="quote"
                  @click.stop="toQuote(purchase.id)"
                />
              </div>
              <div style="text-align: center;">
                <span class="branch">{{purchase.offer_count}}支</span>
              </div>
            </div>
          </div>
          <div class="company">
            ...
            <span class="company_b">{{purchase.send_date}}</span>
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    selectImg() {
      ...
      var data = JSON.parse(JSON.stringify(this.purchaseList));
    ...
    loadMore() {
      ...
      var username = this.userName;
      this.$http({
        url: `${url.purchase}/search/${this.pageNo}/${this.pagesize}`,
        // headers:this.headers,
        data: this.searchMap,
        method: "post"
      }).then(res => {
        if (res.data.code == 20000) {
          var purchaseList = res.data.data.rows;
          for (var i = 0; i < purchaseList.length; i++) {
            purchaseList[i].send_date = this.resolvingDate(
              purchaseList[i].send_date
            );
            purchaseList[i].content_img = JSON.parse(
              purchaseList[i].content_img
            );
          }
          if (purchaseList.length == 0) {
            this.end = true;
          }
          this.purchaseList = this.purchaseList.concat(purchaseList);
          this.updateCount = 0;
        }
      });
    },
    ...
    toQuote(qid) {
      if (!localStorage.getItem("username")) {
        sessionStorage.setItem("toQuote", "toQuote");
      ...
      if (localStorage.getItem("roles").indexOf("provider") == -1) {
        ...
        this.wenz =
          ...
          "，您还不是我们的供应商会员，申请成为供应商会员之后，可以查看平台上所有经销商的信息和进行采购报价，快来申请吧~";
