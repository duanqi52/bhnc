        ...
        <ul>
          ...
          <li>
            <div>
              ...
              <p
                style="flex: 1;text-align: left;color: #ff0000;font-size: 12px;line-height: 1;font-weight:600;margin-left: 165px;"
                v-if="dealerTypeWarn"
              >
          ...
          <!-- <li style="margin-bottom: 30px;">
              ...
              <div class="upfile-box">
                <div class="up-file">
                  <img
                    :src="addImg"
                    alt
                    @click="onPickFileshoquan()"
                    v-if="!user.d_account_img"
                    style="width:100%;height:116px;cursor: pointer;"
                  />
                  <div class="banner-img" v-else style="position:relative">
                    <img :src="user.d_account_img" style="width:100px;height:116px;" />
                    <img
                      :src="removeImg"
                      class="remove"
                      @click="offImg5()"
            ...
            <div style="align-self: flex-end;line-height: 1.5;padding-left:20px;">
              点击下载
              <a
                href="../../../static/img/qiyezhanghaoshouquanshu.doc"
                download="企业帐号授权书"
                style="color:#ff6e0e;"
              >&nbsp;&nbsp;授权书模版&nbsp;&nbsp;</a>填写之后盖公章，再上传电子照片或扫描件
            </div>
            <i class="startMark" style="align-self: flex-end;line-height: 1;">﹡</i>
          </li>-->
          <!-- <li style="margin-bottom: 30px;">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      disabled: false,
      checkVal: "发送验证码",
      user: { qq: [], is_public: "0", dealer_type:[] },
      options: [], //经营范围
  ...
  methods: {
    ...
    selectAll(val) {
      if (val.length > 0) {
        ...
        this.dealerTypeWarn = "采购商分类必须填一个";
      }
        ...
        .then(res => {
          if (res.data.code == 20000) {
            ...
            this.user.roles.map(item => {
              ...
              if(localStorage.getItem("isDealer")=="1"){
                  this.provider = false;
                //this.providerShow = false;
              }
            });
    ...
    amendUser(id) {
      ...
      if (this.user.dealer_type == null || this.user.dealer_type.length == 0) {
        this.warn("采购商分类不能为空");
        this.dealerTypeWarn = "采购商分类必须填一个";
        this.toFocus("company_name");
          ...
          this.$http({
            ...
            if (res.data.code === 20000) {
              ...
              if (roles.indexOf("common") > -1) {
                ...
                localStorage.roles = roles + " dealer";
              }
    ...
    // checkInput(event) {
    //   var wxOne = /^[a-zA-Z0-9]{1}$/;
    //   if (!wxOne.test(event.key)) {
    ...
    clickTwoInput() {
      setTimeout(() => {
        ...
        if (is2) {
          ...
          this.providerTypeWarn = "采购商分类必须填一个";
        }
      }, 10);
    },
    clickOneInput(event) {
    ...
    // 授权证书上传事件
    onPickFileshoquan() {
      document.querySelector(`#shoquanfileD`).click();
    }
    //关闭图片重新选
    // offImg5() {
