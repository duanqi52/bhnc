...
v-loading="submitLoading">
  <div class="registerBox">
    ...
    <div class="amend-user w">
      <div class="amend-msg">
        <ul>
          <!-- <li class="changeshenfen">
            <span>申请身份：</span>
            <label class="identifyCK" v-if="this.dealerShow">
              <i>
                <i v-show="dealer"></i>
              </i>
              <input
                ...
                style="width: 15px;"
                hidden
                :disabled="!this.providerShow"
              />
              <strong>经销商</strong>
            </label>
            <label v-if="this.providerShow">
              <i>
                <i v-show="provider"></i>
              </i>
              <input
          ...
          <li>
            <span>地区：</span>
            <div>
              <span class="user-city" style="padding-right:0;">{{user.region}}</span>
              <el-cascader
                size="large"
                :options="citys"
          ...
          <li style="display:flex">
            ...
            <div>
              <input
                ...
                @blur="checkFrom(5)"
                id="linkman"
                :style="linkmanWarn? 'border: 1px solid #f56c6c;':''"
              />
              <i class="startMark">﹡</i>
              <p style="line-height:1;font-size:12px;color:#b5b5b5;     text-align: left;">（对外公开联系人）</p>
              <p
              ...
              <div style="overflow: hidden;">
                <li style="height:60px;" v-for="(phoneItem,mindex) in mobilePhone" :key="mindex">
                  <input
                    type="text"
                    @keypress="checkNum($event)"
                    v-model="mobilePhone[mindex]"
                    placeholder="请输入手机号码"
                    @blur="checkFrom(6)"
                    :id="'mobilePhone'+mindex"
                  />
                  <i class="startMark" v-if="mindex==0">﹡</i>
                </li>
                <p
                  style="flex: 1;text-align: left;color: #ff0000;font-size: 12px;line-height: 1;font-weight:600"
                  v-if="mPhoneWarn"
            ...
            <div style="overflow: hidden;">
              <li style="height:60px;" v-for="(phone,index) in fixPhone" :key="index">
                <input
                  type="text"
                  @keypress="checkNum($event)"
                  v-model="fixPhone[index]"
                  placeholder="请输入固定电话,格式:XXXX-XXXXXXXX,如010-88888888"
                />
              </li>
            </div>
          ...
          <!-- <li>
            <span>QQ：</span>
            <input type="text" v-model="user.qq[0]" placeholder="请输入QQ号" />
          </li>-->
          <li style="position: relative;display: flex;align-items: flex-start;">
          ...
          <li class="classifyBox clearfix" v-if="providerScopeList.length > 0">
            ...
            <div class="theSecondClassify" v-if="providerScopeList[oneIndex].provider_two_scopes">
              <h3>二级分类</h3>
              <div class="scrollBox" style="height:600px; overflow-y: scroll;">
                <ul>
                  <li
                  ...
                  >
                    ...
                    <span class="classifyName" style="width:auto;">{{providerTwoScope.name}}</span>
                  </li>
                </ul>
              </div>
            </div>
          </li>
          <li class="changeshenfen" v-if="provider">
            <div>
              <div style="display: flex;">
                ...
                <label>
                  ...
                  <input type="checkbox" v-model="pattern1" style="width: 15px;" hidden />
                  <strong>加工制作</strong>
                </label>
                <label>
                  <i class="iCheck">
                  ...
                  <input type="checkbox" v-model="pattern2" style="width: 15px;" hidden />
                  <strong>自有品牌运营</strong>
                </label>
                <label>
                  <i class="iCheck">
                    <i v-show="pattern3">✔</i>
                  </i>
                  <input type="checkbox" v-model="pattern3" style="width: 15px;" hidden />
                  <strong>专利产品授权生产</strong>
                </label>
              </div>
              <div style="margin-left: 165px;" v-if="!(pattern3 || pattern2 || pattern1)">
                <p
                  style="flex: 1;text-align: left;color: #ff0000;font-size: 12px;line-height: 1;font-weight:600"
                >
                  <i class="iconfont icon-guanbi2"></i>
          ...
          <li class="companyProfile">
            ...
            <textarea
              style="resize:none"
              v-model="user.company_introduction"
          ...
          <!-- <li class="isok_show">
            ...
            <label for="checkbox">
              ...
              阅读并同意
              <router-link to="/agreementApply_a">
    ...
    <Footers style="border-top: 1px solid #FF0000;"></Footers>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onPickFileshoquan() {
      document.querySelector(`#shoquanfile`).click();
    },
    //获取当前用户信息
    getUserMsg() {
      if (!localStorage.getItem("username")) {
      ...
      this.$http({
        ...
        .then(res => {
          if (res.data.code == 20000) {
            ...
            this.user.is_public = "0";
            this.user.scope = JSON.parse(this.user.scope);
            //   this.qqArr = JSON.parse(this.user.qq);
            if (
              JSON.parse(this.user.qq) == null ||
              JSON.parse(this.user.qq).length == 0
            ) {
              this.qqArr = [""];
            } else {
              this.qqArr = JSON.parse(this.user.qq);
            ...
            if (
              JSON.parse(this.user.fixPhone) == null ||
              JSON.parse(this.user.fixPhone).length == 0
            ) {
              this.fixPhone = [""];
            } else {
            ...
            if (
              JSON.parse(this.user.phone_more) == null ||
              JSON.parse(this.user.phone_more).length == 0
            ) {
              this.mobilePhone = [""];
            } else {
            ...
            if (!(this.pattern1 || this.pattern2 || this.pattern3)) {
              this.pattern1 = true;
            }
          }
        })
    ...
    amendUser(id) {
      ...
      if (!this.user.linkman) {
        this.warn("业务联系人不能为空");
        this.linkmanWarn = "业务联系人不能为空";
        this.toFocus("linkman");
        return;
      }
      if (!this.mobilePhone[0]) {
        this.warn("移动电话不能为空");
        this.mPhoneWarn = "移动电话不能为空";
        this.toFocus("mobilePhone0");
        return;
      }
      ...
      if (user.provider_type.length == 0) {
        this.warn("供应商经营类型必须选一个");
        this.providerTypeWarn = "供应商经营类型必须填一个";
        this.toFocus("wx");
        document.getElementById("wx").blur();
        return;
      }
      if (!(this.pattern1 || this.pattern2 || this.pattern3)) {
        this.warn("经营方式必须选一个");
        this.toFocus("company_introduction");
        document.getElementById("company_introduction").blur();
        return;
      }
      ...
      //   if (!this.user.p_account_img) {
      //     this.warn("请上传企业帐号授权书");
      //     return;
      //   }
      var otherImgs = [];
      ...
      this.$confirm("是否提交？", "提示", {
        ...
        .then(res => {
          this.$http({
            ...
            if (res.data.code === 20000) {
              this.$message({
                message: "提交成功!审核通过前以体验供应商身份登录",
                type: "success",
                offset: 550
              });
              var roles = localStorage.getItem("roles");
              if (roles.indexOf("common") > -1) {
                localStorage.roles = "provider";
              } else {
                localStorage.roles = roles + " provider";
              }
    ...
    checkFrom(index) {
      ...
      //   if (!this.user.p_account_img) {
      //     this.provideImgWarn = "请上传企业帐号授权书";
      //   } else {
      //     this.provideImgWarn = "";
      //   }
    },
    ...
    // 添加固话数组
    addPhone() {
      if (this.fixPhone.length >= 5) {
        if (document.getElementsByClassName("el-message").length == 0)
          this.$message.error("固话不能超过5个!");
        return;
      }
      this.fixPhone.push("");
    ...
    //关闭图片重新选
    // offImg5() {
    //   this.user.p_account_img = "";
    //   this.checkFrom();
    //   this.$forceUpdate();
    // },
    // imgUpload5(event, index) {
    //   var formData = new FormData();
    ...
    //     headers: { "Content-Type": "multipart/form-data" }
    //   }).then(res => {
    //     this.user.p_account_img = res.data;
    //     this.checkFrom();
    //     this.$forceUpdate();
    //   });
    // },
    // 增加手机数组
    addMPhone() {
      if (this.mobilePhone.length >= 5) {
        if (document.getElementsByClassName("el-message").length == 0)
          this.$message.error("业务联系移动电话不能超过5个!");
        return;
      }
    ...
    // 增加qq数组
    addQQArr() {
      if (this.qqArr.length >= 5) {
        if (document.getElementsByClassName("el-message").length == 0)
          this.$message.error("QQ号码不能超过5个!");
        return;
      }
      this.qqArr.push("");
    ...
    checkNum(event) {
      var num = /^[0-9]{1}$/;
      var jian = new RegExp("-{1}");
      var jia = new RegExp("\\+{1}");
      if (
        !(num.test(event.key) || jian.test(event.key) || jia.test(event.key))
      ) {
  ...
  updated() {
    if (this.user.company_name) {
      sessionStorage.setItem("companyName", this.user.company_name);
    }
  }
};
</script>
