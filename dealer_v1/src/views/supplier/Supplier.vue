            ...
            <div v-show="show3" style="padding: 13px; border: 1px solid #fc9263;">
              ...
              <div>
                ...
                <el-checkbox-group v-model="checkList">
                  <el-checkbox label="加工制作"></el-checkbox>
                  <el-checkbox label="自有品牌运营"></el-checkbox>
                  <el-checkbox label="专利产品授权生产"></el-checkbox>
                </el-checkbox-group>
              </div>
              <div>
                <p class="region_a">经营分类</p>
                <el-checkbox
      ...
      >
        ...
        <el-table-column label="操作" align="see">
          <template slot-scope="scope">
            ...
            <button class="seedetails" @click="$router.push('/supplierDetail?index=0&id='+scope.row.id)">查看详情</button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    search(page, size) {
      ...
      this.searchMap.provider_type = JSON.stringify(this.checkedCities);
    ...
    getProviderScope() {
      this.$http({
        // url: `http://182.61.148.51:9008/scope/providerScope`,
        url: url.getProviderScope,
        method: "get"
      }).then(res => {
        if (res.data.code == 20000) {
