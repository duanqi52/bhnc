...
<template>
  <div class="supplierDetail">
    <div class="supplierDetailTitle">供应商详情</div>
    <!-- 内容部分 -->
    <div class="supplierDetailContent">
      <ul>
        ...
        <li class="clearfix">
          <div>
            ...
            <i v-else style="font-size:12px; font-family:MicrosoftYaHei;font-weight:400;">(不公开)</i>
          </div>
          <div>
            <span>法人移动电话：</span>
            <div>
      ...
      <div class="manageType">
        <div class="forBox clearfix">
          <div class="manageTypeTitle fl">供应商经营类型：</div>
          <div class="manageTypeContnet fl">
            <ul v-if="providerDetails.provider_type">
              <div v-for="(item,index) in providerDetails.provider_type" :key="index">
                <li v-if="item.name != ''">
                  <p>{{item.name}}：</p>
                  <p>{{item.data.join("、")}}</p>
                </li>
        ...
        <hr color="#f0f0f0" style="width:80%;" align="left" />
        <div class="jingyingf" style="padding:32px 0;">
          <span>经营方式：</span>
          <i>{{providerDetails.pattern}}</i>
        </div>
      </div>
    </div>
    <!-- 供应商品列表 -->
    <div class="shopList">
      <div class="shopListTitle clearfix">
        <h3 class="fl">供应商品</h3>
        <i class="fr" @click.stop="toMore()">更多>></i>
      </div>
      <div class="shopListContent">
        <ul class="clearfix">
          <li v-for="(item,index) in providerGoodsList" :key="index" @click.stop="toDetail(item)">
            <div>
...
export default {
  ...
  methods: {
    ...
    toMore(){
      var href = `${url.shopUrl}/allGoods?providerId=`+this.$route.query.id;
      window.open(href);
    },
    getDetails() {
      this.$http({
        ...
        if (res.data.code == 20000) {
          ...
          if (this.providerDetails.provider_type)
            this.providerDetails.provider_type = JSON.parse(
              this.providerDetails.provider_type
            );
        }
      });
    },
    ...
    lookProvider(pid) {
      ...
      if (this.providerDetails.level == "1") {
        this.$confirm("您的级别没有权限查看供应商信息，请充值积分达到2级会员！","提示",{
      ...
      } else if(this.providerDetails.level=='89'){
        var level="";
        for (var i = 0; i < this.setLevel.length; i++) {
         ...
         if(level=="1"){
            this.$confirm(
                "您的级别没有权限查看供应商信息，请充值积分达到2级会员！","提示",{
                  confirmButtonText: "去充值",
                  cancelButtonText: "取消",
                  type: "warning",
                  center: true
                ...
                this.$confirm(
                  `本次查看供应商信息将消耗${this.providerDetails.look_provider}积分，是否继续查看？`,"提示",{
                  ...
                  this.$http({
                    ...
                    url: `${url.addDealerLookProvider}/${localStorage.getItem("ID")}/${pid}`
                  }).then(res => {
                    this.getDetails();
                  });
                });
    ...
    star(pid, collect) {
      if (collect == 1) {
        this.$http({
          method: "get",
          url: `${url.addCollect}/${localStorage.getItem("ID")}/${pid}/0`
        }).then(res => {
          this.getDetails();
        });
      } else {
        this.$http({
          method: "delete",
          url: `${url.deleteCollect}/${localStorage.getItem("ID")}/${pid}/0`
        }).then(res => {
          this.getDetails();
        });
      }
    ...
    updatePratner(pid, type) {
      if (type == 1) {
        this.$http({
          method: "get",
          url: `${url.addPartner}/${localStorage.getItem("ID")}/${pid}/0`
        }).then(res => {
          this.getDetails();
        });
      } else {
        this.$http({
          method: "delete",
          url: `${url.deletePartner}/${localStorage.getItem("ID")}/${pid}/0`
        }).then(res => {
          this.getDetails();
        });
      }
