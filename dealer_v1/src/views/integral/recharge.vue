    ...
    <div class="rechargeBox">
      ...
      <div class="payBox">
        ...
        <div class="chargeInput">
          <input type="text" placeholder="请输入充值金额" v-model="amount" class="inputVal" min="1" 
     ...
     // return
      ...
      this.payType.forEach((item, index) => {
        if (item.isCheck == true && "支付宝支付" == item.payName) {
          ...
          this.$http({
            //url: 'http://localhost:8888/pay/supplier/alipay/sub',
            url: `${url.payDealerIntegralBuy}`,
            method: "post",
            data: data
          }).then(res => {
        ...
        if (item.isCheck == true && "微信支付" == item.payName) {
          ...
          this.amount = amount;
          this.$http({
            // url: 'http://localhost:8888/wxpay/dealerIntegralWxPay',
            url: `${url.dealerIntegralWxPay}`,
            method: "post",
            data: data
          }).then(res => {
            if (res.data.code == 20000) {
              this.paymentInfo = res.data.data;
              // this.wxUrl='http://localhost:8888/wxpay/dealerIntegralWxPayQrcode/'+this.paymentInfo.total_amount+"/"+this.paymentInfo.trade_no;
              this.isShow = true;
              this.wxUrl =
                url.dealerIntegralWxPayQrcode +
                "/" +
