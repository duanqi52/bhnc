                 ...
                 <span class="fl">{{item.title}}</span>
                  <div class="checkboxRight fr">
                    <input
                      ...
                      :id="`addShop` + item.id"
                    />
                    <label :for="`addShop` + item.id"></label>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getMarketingGoods(page, size) {
      this.$http
        .get(
          `${url.getMarketingToolGoods}/${localStorage.getItem(
            "siteName"
          )}/${page}/${size}`
        )
        .then(res => {
    ...
    toDetail(goods) {
      // if (goods.is_pay == "1") {
      //   var href="http://182.61.148.122:30/detail?id="+goods.id;
      //   window.open(href);
      // } else {
      //   var href="http://182.61.148.122:30/detailSimple?id="+goods.id;
      //   window.open(href);
      // }
      ...
      if (goods.is_pay == "1") {
        ...
        urlID = "simple" + goods.id;
      }
      // var url = "http://182.61.148.122:40/saveParam.html?id=" + urlID;
      var urls = `${url.detailUrl}/saveParam.html?id=` + urlID;
      if (username) {
      ...
      if (VISITOR) {
        urls += "&VISITOR=" + VISITOR;
      }
      if (Authorization) {
        urls += "&Authorization=" + Authorization;
      }
      if (ID) {
        urls += "&UID=" + ID;
      }
