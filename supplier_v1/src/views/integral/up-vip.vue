<!-- 组件说明 -->
<template>
  <div id="bhncUp-vip" style="background:#f0f0f0">
    <div class="integral">
      <div class="bg_ji">
        <div class="integral-msg">
          ...
          <div
            style="font-size:18px;color:rgba(102,102,102,1);margin-top: 80px;text-align: right;padding-right: 30px;"
        ...
        <div class="integral-msg_a">
          ...
          <p>
            白银会员
            <br />用户注册成为“笔潮弄潮”的采购商会员、供应商会员，即自动成为白银会员。白银会员可在使用平台上大部分的功能，例如收藏商品、收藏采购商、收藏供应商、编辑发布企业营销目录，以及发布求购信息、供应信息等操作。
            <br />黄金会员
            <br />用户注册成为“笔潮弄潮”的采购商会员、供应商会员后，当积分累积达到2001分后，即自动成为黄金会员。
    ...
    <div class="up-vip">
      ...
      <div id="mask" ref="mask" v-show="isShow" @touchmove.prevent>
        ...
        <div class="payImg" @click.stop>
          ...
          <div class="wechatBox" v-if="isWechat">
            ...
            >至尊VIP年费会员:{{supplierSetLevel.money}}元/年</div>
            <div class="codeBg">
              <!-- <img  src="/static/img/yard.png" alt
      ...
      <div
        style="display: inline-block;position: absolute;width: 55%;left: 3%;top: 20%;min-height: 227px;"
      >
        <p>
          至尊VIP会员
          <br />用户注册成为“笔潮弄潮”的采购商会员、供应商会员后，通过付费升级，可以成为“笔潮弄潮”的至尊VIP会员。
          <br />至尊VIP会员可在使用平台上全部的功能，包括但不限于收藏商品、收藏其他采购商、收藏其他供应商、编辑发布企业营销目录、查看目标采购商信息、查看目标供应商信息，以及回复求购信息报价、发布广播消息等操作，且至尊VIP会员进行任何操作都不消耗任何积分，同时享受“笔潮弄潮”的VIP服务待遇。
        </p>
      </div>
      <div class="vip-box">
        <!-- <img :src="vipImg" alt class="up_vip_img" @click.stop="isShow = true" v-if="level !='89'" /> -->
...
export default {
  ...
  mounted() {
    ...
    document.getElementById("bhncUp-vip").style.height = windowHeight + "px";
  },
  methods: {
    getSupplierSetLevel() {
      this.$http({
        // url:'http://localhost:9004integralRecord/getSupplierSetLevel',
        url: `${url.getSupplierSetLevel}`,
        method: "get"
      }).then(res => {
        if (res.data.code == 20000) {
          this.supplierSetLevel = res.data.data;
        }
      });
    },
    getSetLevel() {
    ...
    getUserMsg() {
      this.$http({
        ...
        .then(res => {
          ...
          if (this.user.level == "89" || this.user.level == "100") {
            this.level = "至尊VIP会员";
          }
    ...
    toYearMember() {
      this.$http({
        url: `${url.toYearMember}/${localStorage.getItem("username") || localStorage.getItem("siteName")}/provider`,
        method: "get"
      }).then(res => {
        if (res.data.code == 20000) {
          this.getIntegral();
    ...
    toWechatPay() {
      ...
      this.isWechat = true;
      var data = {};
      data.amount = this.supplierSetLevel.money;
      data.uid = localStorage.getItem("ID");
        this.$http({
        //url: 'http://localhost:8888/wxpay/supplierAnnualWxPay',
        url: `${url.supplierAnnualWxPay}`,
        method: "post",
        data: data
      }).then(res => {
           if (res.data.code == 20000) {
              this.paymentInfo = res.data.data;
       // this.wxUrl='http://localhost:8888/wxpay/supplierAnnualWxPayQrcode/'+this.paymentInfo.total_amount+"/"+this.paymentInfo.trade_no;
            this.wxUrl=url.supplierAnnualWxPayQrcode+"/"+this.paymentInfo.total_amount+"/"+this.paymentInfo.trade_no;
...
          // url: 'http://localhost:8888/pay/queryorder/'+this.paymentInfo.trade_no,
    ...
    toAlipay() {
     // return
      ...
      data.uid = localStorage.getItem("ID");
      this.$http({
        //url: 'http://localhost:8888/pay/supplier/annual',
        url: `${url.paySupplierAnnual}`,
        method: "post",
        data: data
      }).then(res => {
...
#bhncUp-vip {
  .integral {
    .integral-msg {
      ...
      background: url("../../../static/img/upVip_bg.png") no-repeat;
