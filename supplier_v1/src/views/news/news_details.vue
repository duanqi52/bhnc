<template>
  <div id="bhncNews_details">
    ...
    <div class="weitu" v-if="dShow == '1' ">
      <div>
        <p class="respect">{{purchaseDetails.content}}</p>
        <br />
        <div class="respects">
          <span class="respect_a">
            "
            <span style="text-decoration:underline;cursor: pointer;"  @click=" toDetails()">{{purchaseDetails.url.title}}</span> "
          </span>
      ...
      this.$http({
        ...
        if (res.data.code == 20000) {
          ...
          if (this.purchaseDetails.url) {
            ...
            var content = ""
            if (utype == "1") {
              var username = localStorage.getItem("username") || localStorage.getItem("siteName");
              var roles = localStorage.getItem("roles");
              var Authorization = localStorage.getItem("Authorization");
              var VISITOR = localStorage.VISITOR;
              var ID = localStorage.ID;
              // href = `${url.detailUrl}/saveParam.html?id=` +'simple'+uId;
              href = `${url.detailUrl}/details?id=` + uId;
              if (username) {
            ...
            if (utype == "2") {
              ...
              this.dShow = "1";
              this.detailsShow = "1";
              content = "尊敬的"+localStorage.getItem('siteName')+"会员，您发布的采购信息，已通过审核"
            }
            if (utype == "3") {
            ...
            if (href) {
              ...
              if (utype == "5") {
                ...
                this.dShow = "2";
                this.detailsShow = "0";
                content = "尊敬的"+localStorage.getItem('siteName')+"会员，您发布的采购信息未能通过审核"
              }
              if (utype == "6") {
                this.dShow = "2";
                this.detailsShow = "0";
