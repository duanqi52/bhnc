      ...
      <div class="editSub">
        <ul>
          ...
          <li class="clearfix" style="margin-bottom:20px;">
            ...
            <div class="tipsBox fl">
              <input type="password" placeholder="请输入登陆密码" v-model="userSonSave.old_password" />
          ...
          <li class="clearfix">
            ...
            <div class="tipsBox fl">
              <input type="text" placeholder="请输入移动电话" v-model="userSonSave.phone"  maxlength="11" oninput = "value=value.replace(/[^\d]/g,'')"/>
            </div>
          </li>
          <li class="clearfix">
...
export default {
  ...
  methods: {
   ...
   startClick(row) { 
    ...
    findUserSons() {     
      this.$http({
        url: `${url.findUserSons}/${localStorage.getItem("ID")}/1`  //参数1: 用户ID   参数2: 角色类型   201928
      }).then(res => {
        if (res.data.code == 20000) {
    ...
    updateUserSon() {
      ...
      if (!/^(((13[0-9])|(14[5-7])|(15[0-9])|(17[0-9])|(18[0-9]))+\d{8})$/.test(this.userSonSave.phone)) {
        ...
        for (let index = 0; index <  this.userSonList.length; index++) {
              if(this.userSonList[index].phone==this.userSonSave.phone && this.userSonList[index].id!=this.userSonSave.id){
              this.$message.error("移动电话已被使用");
              return;
          }        
        }     
      if (!this.job) {
      ...
      this.$http({
        //url:'http://172.16.0.138:9007/user/updateByUserSon',
        url: `${url.updateByUserSon}`,
