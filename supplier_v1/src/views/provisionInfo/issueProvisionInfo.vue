    ...
    <div class="userPubilc">
      ...
      <div class="userPublicContent">
        <ul>
          <li>
            <span>标题：</span>
            <input type="text" placeholder="请输入标题" v-model="supplyInfo.title" />
          </li>
          <li>
            <span>商品名称：</span>
            <input type="text" placeholder="请输入商品名称" v-model="supplyInfo.goods_name" />
          </li>
          <li>
            <span>供应数量：</span>
            <input
          ...
          <li style="display:flex;">
            <span>供应说明：</span>
            <textarea placeholder="请输入供应说明，限定500字。" v-model="supplyInfo.content"></textarea>
          </li>
      ...
      <div class="commitStatus">
        ...
        <input type="button" value="提交" style="background:#FC9263" @click="addSupplyInfo()" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    document.getElementById("bhncIssueProvisionInfo").style.minHeight =
      bodyHeight + "px";
  },
  component: {
    editor() {
  ...
  methods: {
    ...
    checkOfferCountInputData() {
      setInterval(() => {
        if (this.oldCount == this.supplyInfo.count) {
          return;
        }
        var numReg = new RegExp("[0-9]{1}");
        var value = this.supplyInfo.count;
        // console.log(value);
        var valueSave = "";
        ...
        this.supplyInfo.count = valueSave;
    ...
    addSupplyInfo() {
      ...
      if (document.querySelectorAll(".el-message--warning").length < 1) {
        if (!this.supplyInfo.title) {
          this.warn("标题");
          return;
        }
        if (!this.supplyInfo.goods_name) {
          this.warn("商品名称");
          return;
        }
        if (!this.supplyInfo.count) {
          this.warn("供应数量");
          return;
        }
        if (!this.supplyInfo.price) {
          this.warn("商品单价");
          return;
        }
        if (!this.supplyInfo.content) {
          this.warn("描述");
          return;
        }
        this.supplyInfo.u_id = localStorage.getItem("ID");
        var imgs = [];
        this.imgs.map(item => {
          if (item.img) {
            imgs.push(item.img);
        ...
        this.supplyInfo.imgs = JSON.stringify(imgs);
        this.$http({
          url: `${url.addSupplyInfo}`,
          method: "post",
          data: this.supplyInfo
        })
          .then(res => {
            if (res.data.code == 20000) {
              this.$router.push({ path: "/provisionInfo" });
            } else {
              this.$message({
